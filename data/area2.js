const AREA2_TOPICS = [
  ...
  },
  {
    ...
    note: "En programas como Word o Google Docs hay iconos específicos para dar formato. **Insertar tabla**: crea una cuadrícula de filas y columnas para organizar datos, suele estar en la pestaña \"Insertar\". **Ajustar márgenes**: controla el espacio en blanco alrededor del texto en la página, está en \"Diseño\" o \"Formato\". **Dividir texto en columnas**: separa el contenido en dos o más columnas (como un periódico), se encuentra en \"Diseño\" > \"Columnas\". Reconocer estos iconos es clave, y el examen los mezcla con otros del mismo menú, así que conviene tener los seis identificados por su dibujo: **tabla**, una cuadrícula de celdas vacías; **columnas**, una hoja con el texto partido en dos franjas verticales; **márgenes**, la hoja completa con un marco punteado por dentro (ese marco es el área de texto y lo de fuera es el margen); **salto de página**, dos hojas separadas por una línea punteada, que manda todo lo que sigue a la hoja siguiente y lo mantiene ahí aunque se edite lo anterior —bajar con Enter parece igual pero se desacomoda—; **viñetas**, unos puntos con renglones al lado, para listas; e **interlineado**, unos renglones con flechas verticales al lado, que cambia el espacio ENTRE renglones. No confundas margen con **sangría**: el margen se fija para toda la página desde Configurar página y la sangría solo desplaza un párrafo dentro de esos márgenes.",
    flashcards: [
