const AREA1_TOPICS = [
  ...
  },
  {
    ...
    note: "Las tres medidas de tendencia central resumen \"dónde está el centro\" de un conjunto de datos. **Media** = suma de todos los datos entre el número de datos. **Mediana** = el valor de en medio al ordenar los datos de menor a mayor (si son pares, se promedian los dos de en medio). **Moda** = el valor que más se repite. Ejemplo: notas 7,8,9,9,9,10. Media = (7+8+9+9+9+10)/6 ≈ 8.67. Ordenados: mediana = promedio del 3er y 4to valor = (9+9)/2 = 9. Moda = 9 (se repite 3 veces). Truco para el examen: si piden \"el promedio\" usa media; si piden \"el de en medio\" usa mediana; si piden \"el más frecuente\" usa moda. Cuidado con \"el valor típico\": no siempre es la media. Cuando hay datos muy extremos o muy desiguales —un sueldo de 200 000 entre sueldos de 8 000— la media se va detrás del dato extremo y la **mediana** representa mejor al conjunto. Si el reactivo subraya que hay un dato atípico o que la distribución está desbalanceada, la respuesta es mediana.",
    flashcards: [
  ...
  },
  {
    ...
    note: "La probabilidad de un evento es cuántos resultados favorables hay entre el total de resultados posibles: P = (casos favorables) / (casos totales). Se puede expresar como fracción, decimal o porcentaje (multiplicando por 100). Ejemplo: una caja tiene 4 canicas rojas y 6 azules (10 en total). La probabilidad de sacar una azul es 6/10 = 0.6 = 60%. Si lanzas un dado normal de 6 caras, la probabilidad de que caiga un número par (2,4,6) es 3/6 = 0.5 = 50%. También entra la **probabilidad complementaria**: la de que un evento NO ocurra es P(no A) = 1 - P(A). Cuando la pregunta dice «que NO sea...», suele salir más rápido contar los casos que sí cumplen y restarlos del total. Ejemplo: en una bolsa con 3 rojas, 7 azules y 4 verdes (14 en total), P(no verde) = 1 - 4/14 = 10/14 = 5/7. Truco: identifica primero el total de casos posibles (todo el espacio) y luego cuenta solo los casos que cumplen la condición que piden.",
    flashcards: [
  ...
  },
  {
    ...
    note: "La probabilidad condicional calcula la probabilidad de un evento SABIENDO que ya ocurrió otro. El **teorema de Bayes** permite \"voltear\" la condición: P(A|B) = [P(B|A) × P(A)] / P(B). Ejemplo: una fábrica tiene dos máquinas: la Máquina A hace el 60% de las piezas con 5% defectuosas, la Máquina B hace el 40% con 10% defectuosas. Si eliges una pieza defectuosa al azar, ¿cuál es la probabilidad de que venga de la Máquina A? Primero P(defectuosa) = 0.6×0.05 + 0.4×0.10 = 0.07. Luego P(A|defectuosa) = 0.03/0.07 ≈ 42.9%. Otros dos casos muy frecuentes. **Extracción sin reemplazo**: al no regresar lo que sacaste cambian tanto los casos favorables como el total. Ejemplo: en una caja con 6 plumas rojas y 6 azules, si sacas una roja y no la regresas, la probabilidad de que la siguiente también sea roja es 5/11, porque quedan 5 rojas de 11 plumas. **Tabla de doble entrada**: la condición reduce el espacio muestral a un solo renglón o columna, y dentro de ese grupo se aplica favorables entre totales. Truco: primero calcula la probabilidad total del evento sumando todos los caminos posibles, y luego divide solo el camino que te interesa entre ese total.",
    flashcards: [
  ...
  },
  {
    ...
    note: "Una expresión algebraica traduce una relación entre dos cantidades usando letras (variables) y números. Identifica primero el **costo o valor fijo** (número solo, no depende de nada) y la **parte que cambia** (número que multiplica a la variable). Ejemplo: un taxi cobra $8 de banderazo (fijo) más $3 por cada km recorrido (k). El total pagado T se representa como T = 8 + 3k. Otro ejemplo: un plomero cobra $300 por visita más $120 por cada hora trabajada (h): el pago total P = 300 + 120h. El otro formato del tema es traducir una frase a símbolos, y ahí lo que se evalúa es leer con cuidado. «El doble» es 2x y «el triple», 3x; «la mitad de un número» es x/2; «el cuadrado de un número» es x²; «el consecutivo de un número» es x+1, así que «la suma de un número con su consecutivo» es x + (x+1); «el 5% de un número» es 5x/100 (en general, ax/100). Los **paréntesis** marcan la diferencia entre dos frases casi iguales: «el doble de un número más 5» es 2x + 5, pero «el doble de la suma de un número con 5» es 2(x + 5), porque primero se suma y después se duplica. Truco: la parte que NO tiene letra es el costo fijo; la parte que multiplica a la variable es la tarifa o costo por unidad.",
    flashcards: [
  ...
  },
  {
    ...
    note: "Factorizar es lo contrario de multiplicar: partes de un polinomio (ej. x²+7x+12) y lo escribes como producto de dos binomios más simples. Para x²+bx+c, busca dos números que MULTIPLICADOS den \"c\" y SUMADOS den \"b\". Ejemplo: x²+7x+12 → buscas dos números que multiplicados den 12 y sumados den 7: son 3 y 4. Entonces x²+7x+12 = (x+3)(x+4). Otro ejemplo: x²-5x+6 → buscas números que multiplicados den 6 y sumados den -5: son -2 y -3. Entonces x²-5x+6 = (x-2)(x-3). Antes de buscar los dos números, revisa otros dos casos. **Factor común**: si todos los términos comparten algo, se saca **todo** lo que comparten. Ejemplo: 6x²+18x = 6x(x+3). Cuidado con quedarse a medias: 3x(2x+6) también es cierto, pero no es factorizar POR COMPLETO, porque 2x+6 todavía tiene el 2 como factor común. **Diferencia de cuadrados**: a²-b² = (a+b)(a-b). Ejemplo: 9x²-25 = (3x)²-5² = (3x+5)(3x-5). Ojo: una SUMA de cuadrados no se factoriza. Truco: siempre puedes comprobar tu resultado multiplicando de vuelta los binomios para ver si te da el polinomio original.",
    flashcards: [
  ...
  },
  {
    ...
    note: "Los productos notables son fórmulas rápidas para elevar binomios al cuadrado: (a+b)² = a²+2ab+b² y (a-b)² = a²-2ab+b². Sirven para \"completar el cuadrado\" y encontrar el valor máximo o mínimo de una función cuadrática sin usar cálculo: eso es justo lo que el examen llama un problema de **optimización** de funciones polinomiales de segundo grado. Pasos: convierte x²+bx+c a la forma (x-h)²+k, donde el mínimo (o máximo si el signo es negativo) es k, alcanzado en x=h. Ejemplo: x²-6x+5. Como (x-3)²=x²-6x+9, entonces x²-6x+5 = (x-3)²-4. El valor mínimo es -4 y ocurre en x=3. Hay otros dos productos notables que caen seguido. **Binomios conjugados**: (a+b)(a-b) = a²-b², la diferencia de cuadrados. Ejemplo: (x+8)(x-8) = x²-64. **Binomios con término común**: (x+a)(x+b) = x²+(a+b)x+ab. Ejemplo: (x+7)(x+5) = x²+12x+35. Truco: si el coeficiente de x² es positivo, la función tiene un mínimo; si es negativo, tiene un máximo.",
    flashcards: [
  ...
  },
  {
    ...
    subarea: "1.3 Pensamiento algebraico",
    tema: "Interés simple",
    note: "El interés simple se calcula solo sobre el capital inicial, sin acumular. Fórmula: I = C × r × t, donde C=capital, r=tasa (en decimal, ej. 8% = 0.08) y t=tiempo (en años). Para hallar el **plazo**: t = I / (C×r). Para hallar la **tasa**: r = I / (C×t). Ejemplo (plazo): si un capital de $5,000 a tasa anual de 8% genera $600 de interés, el tiempo es t = 600/(5000×0.08) = 1.5 años. Ejemplo (tasa): si $8,000 generan $960 de interés en 2 años, la tasa es r = 960/(8000×2) = 0.06 = 6%. El **monto final** es simplemente el capital más el interés: M = C + I. Con $5,000 al 8% durante 1.5 años, I = $600 y M = $5,600. Truco: siempre convierte el porcentaje a decimal antes de calcular (divide entre 100).",
    flashcards: [
  ...
  },
  {
    ...
    note: "Área del **triángulo** = (base × altura) / 2. Área del **trapecio** = ((base mayor + base menor) / 2) × altura. La altura siempre es la distancia perpendicular (a 90°) entre la base y el vértice o lado opuesto, no un lado inclinado. Ejemplo triángulo: base=10 cm, altura=6 cm → área = (10×6)/2 = 30 cm². Ejemplo trapecio: base mayor=12 cm, base menor=8 cm, altura=5 cm → área = ((12+8)/2)×5 = 50 cm². Completa la lista con las demás figuras que pide el examen: **rectángulo** = base × altura; **cuadrado** = lado²; **círculo** = πr², con π ≈ 3.14. Ejemplo: un círculo de radio 5 cm tiene área 3.14×5² = 78.5 cm². Cuidado con confundir área y perímetro: en el círculo, 2πr es la circunferencia, no el área. El examen también lo pregunta al revés: te da el área y te pide una medida, así que hay que **despejar**. Ejemplo: un trapecio de 36 cm² con altura 4 cm y base menor 3 cm. De área = ((B + b) ÷ 2) × altura se despeja 36 ÷ 4 = 9, así que (B + 3) ÷ 2 = 9, B + 3 = 18 y B = 15 cm. Truco: recuerda que el triángulo siempre se divide entre 2 (es \"medio rectángulo\"), y el trapecio promedia sus dos bases antes de multiplicar por la altura.",
    flashcards: [
  ...
  },
  {
    ...
    note: "Dos triángulos son **congruentes** si tienen exactamente el mismo tamaño y forma (lados y ángulos iguales). Son **semejantes** si tienen la misma forma pero distinto tamaño (ángulos iguales, lados proporcionales). Criterios comunes: **LLL** (los tres lados proporcionales), **LAL** (dos lados y el ángulo entre ellos), **AA** (dos ángulos iguales, suficiente para semejanza). En triángulos semejantes, si conoces la \"escala\" (factor de proporción), puedes hallar lados desconocidos. Ejemplo: triángulo ABC semejante a DEF con escala 2. Si AB=4 y BC=6, entonces DE=8 y EF=12. Dos aplicaciones frecuentes. **Sombras y alturas**: una persona y un árbol medidos a la misma hora forman triángulos semejantes, así que la razón altura/sombra es la misma en ambos. Ejemplo: una persona de 150 cm con sombra de 50 cm y un árbol con sombra de 350 cm → 150/50 = x/350, de donde x = 1,050 cm. **Áreas**: si los lados están en razón k, las áreas están en razón k². Ejemplo: con lados en razón 4:1, el área del grande es 16 veces la del chico. Truco: primero identifica qué lados se corresponden entre los dos triángulos, y calcula la razón entre un par de lados conocidos para usarla en los demás.",
    flashcards: [
  ...
  },
  {
    ...
    note: "Un intervalo en la recta numérica se representa con una desigualdad. **Círculo cerrado (relleno)** significa que el número SÍ está incluido (se usa ≤ o ≥). **Círculo abierto (vacío)** significa que el número NO está incluido (se usa < o >). Si el intervalo está sombreado entre dos puntos, ambas condiciones se combinan. Ejemplo: círculo cerrado en -2 y círculo abierto en 6, con sombreado entre ambos → -2 ≤ x < 6. Ejemplo: círculo abierto en 0 con flecha hacia la derecha sin límite → x > 0. El intervalo también se escribe con **corchete y paréntesis**: el corchete [ ] incluye el extremo (≤ o ≥) y el paréntesis ( ) lo excluye (< o >). Así, -2 ≤ x < 6 se escribe [-2, 6) y x > 0 se escribe (0, ∞); el infinito siempre lleva paréntesis. Para **resolver** una desigualdad se despeja igual que en una ecuación, con una sola excepción: al multiplicar o dividir entre un número **negativo** se invierte el sentido. Ejemplo: -2x + 8 < 20 → -2x < 12 → x > -6. Truco: la flecha te dice hacia dónde sigue el intervalo, y el tipo de círculo te dice si incluir (cerrado) o no (abierto) ese extremo.",
    flashcards: [
  ...
  },
  {
    ...
    note: "Al leer la gráfica de una función puedes identificar varias características sin hacer cálculos. **Máximo**: el punto más alto (la curva sube y luego baja). **Mínimo**: el punto más bajo (baja y luego sube). **Creciente**: la curva sube de izquierda a derecha. **Decreciente**: la curva baja de izquierda a derecha. **Concavidad hacia arriba** (como una \"U\"): forma de mínimo. **Concavidad hacia abajo** (como una \"∩\"): forma de máximo. Ejemplo: una parábola que abre hacia abajo tiene un máximo en su punto más alto y es cóncava hacia abajo en toda su gráfica. Tres cálculos que acompañan a la lectura de la gráfica. **Vértice de una parábola** y = ax²+bx+c: x = -b/(2a), y el valor de y se obtiene sustituyendo esa x. Ejemplo: y = x²-4x+7 → x = 4/2 = 2, y = 3, vértice (2, 3). **Pendiente** de la recta que pasa por dos puntos: m = (y₂-y₁)/(x₂-x₁). **Ordenada al origen**: en y = mx+b, el corte con el eje Y es el punto (0, b). Reconocer la forma por la ecuación: **y = mx+b** es una **recta**; **y = ax²+bx+c** es una **parábola**, que abre hacia arriba si a>0 y hacia abajo si a<0; **y = 2ˣ** y demás potencias de base fija son **curvas exponenciales**, que crecen cada vez más rápido y nunca cortan el eje X. Truco: imagina que caminas sobre la curva de izquierda a derecha: si subes, es creciente; si bajas, es decreciente.",
    flashcards: [
  ...
  },
  {
    ...
    note: "El límite de una función te dice a qué valor se \"acerca\" f(x) cuando x se acerca a un número dado, aunque la función no esté definida exactamente ahí. Para funciones cuadráticas (continuas), casi siempre puedes calcular el límite por **sustitución directa**: sustituye el valor de x en la función. Ejemplo: límite cuando x→3 de (x²+2x-5) = 9+6-5 = 10. Si al sustituir obtienes 0/0 (forma indeterminada), factoriza y simplifica antes de sustituir. Ejemplo: límite cuando x→5 de (x²-25)/(x-5). Como x²-25=(x-5)(x+5), simplificas a (x+5), y sustituyendo x=5: 5+5=10. Falta un tercer caso: el **límite cuando x tiende a infinito** de un cociente de polinomios. Ahí mandan los términos de mayor grado y lo demás se vuelve despreciable. Si los dos grados son iguales, el límite es el cociente de los coeficientes principales. Ejemplo: lím(x→∞) (6x²+8x)/(8x²+6) = 6/8 = 3/4. Si el grado de arriba es menor, el límite es 0; si es mayor, se va a infinito. Truco: siempre intenta la sustitución directa primero; solo factoriza si te da 0/0.",
    flashcards: [
  ...
  },
  {
    ...
    note: "Además de los polinomios, hay funciones con reglas de derivación propias que conviene memorizar. **Raíz cuadrada**: derivada de √x es 1/(2√x). **Trigonométricas**: derivada de sen(x) es cos(x); derivada de cos(x) es -sen(x). **Exponencial**: derivada de e^x es el mismo e^x. **Logaritmo natural**: derivada de ln(x) es 1/x. Ejemplo: f(x)=√x → f'(x)=1/(2√x). Otro ejemplo: f(x) = sen(x) + eˣ → f'(x) = cos(x) + eˣ (derivas cada término por separado, igual que en polinomios). Dos reglas más que hacen falta en cuanto la función deja de ser una sola letra. **Regla de la cadena**: si dentro de la función hay otra función, se deriva la de fuera y se multiplica por la derivada de la de dentro. Ejemplo: f(x)=e^(3x) → f'(x)=3e^(3x); f(x)=sen(5x) → f'(x)=5cos(5x). **Regla del producto**: la derivada de un producto es (primero)'×(segundo) + (primero)×(segundo)'. Ejemplo: f(x)=x·eˣ → f'(x)=1·eˣ + x·eˣ = eˣ(x+1). Y una constante que multiplica simplemente se conserva: f(x)=4ln(x) → f'(x)=4/x. Truco: memoriza estas reglas básicas como una tabla, ya que el examen casi siempre pide aplicarlas directamente, sin combinaciones complicadas.",
    flashcards: [
  ...
  },
  {
    ...
    note: "Para hallar el máximo o mínimo de una función cuadrática con derivadas: 1) deriva la función; 2) iguala la derivada a 0 (ahí está el punto máximo o mínimo, porque la pendiente es plana); 3) despeja x; 4) sustituye ese x en la función original para hallar el valor máximo/mínimo. Ejemplo: la altura de un objeto es h(t)=-5t²+20t+1. Derivando: h'(t)=-10t+20. Igualando a 0: t=2. Sustituyendo: h(2)=-5(4)+40+1=21, la altura máxima es 21 en t=2 segundos. Cuando el problema no te da la función, primero hay que **plantearla**. Ejemplo típico: con 200 m de malla se cerca un terreno rectangular. El perímetro fija x+y = 100, así que y = 100−x y el área es A(x) = x(100−x) = 100x−x². Derivando: A'(x) = 100−2x = 0 → x = 50, y = 50: el rectángulo de área máxima con un perímetro dado siempre es el cuadrado, con área 2,500 m². Truco: si el coeficiente de x² (o t²) es negativo, el punto encontrado es un máximo; si es positivo, es un mínimo.",
    flashcards: [
